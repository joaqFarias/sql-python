# 1. ¿Qué consulta harías para obtener todos los países que hablan esloveno? Tu consulta debe devolver el nombre
# del país, el idioma y el porcentaje de idioma. Tu consulta debe organizar el resultado por porcentaje de idioma 
...
select countries.name, count(cities.country_code) as 'num_cities' from countries
join cities on countries.id = cities.country_id  
group by country_id
order by num_cities desc;

# 3. ¿Qué consulta harías para obtener todas las ciudades de México con una población de más de 500,000? Tu
# consulta debe organizar el resultado por población en orden descendente. (1)

select countries.name, cities.name, cities.population, cities.country_id from countries
join cities on countries.id = cities.country_id
...
countries join languages on countries.id = languages.country_id
where languages.percentage > 89
order by languages.percentage desc;

# 5. ¿Qué consulta haría para obtener todos los países con un área de superficie inferior a 501 y una población
# superior a 100,000? (2)
...
# tener una población superior a 500,000? La consulta debe devolver el nombre del país, el nombre de la ciudad,
...
# 8. ¿Qué consulta harías para resumir el número de países en cada región? La consulta debe mostrar el nombre 
# de la región y el número de países. Además, la consulta debe organizar el resultado por el número de países 
